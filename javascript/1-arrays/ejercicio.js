...
function vaciarCarritoConTotal() {
    ...
    while (carrito.length > 0) {
        // Utiliza shift() para obtener y eliminar el primer elemento del carrito
...
function cuentaDeAlumnos() {
    ...
    // Utiliza concat() para combinar los dos grupos en un nuevo arreglo
    const grupoUnificado = grupoA.concat(grupoB);

    // Imprime en consola el resultado (la cantidad de alumnos)
    console.log(grupoUnificado.length);
}
...
function transmitiendoBatch() {
    ...
    const elementosPorLote = backup.length / 4;

    // Utiliza slice() para mostrar en lotes y muestra en consola
    for (let i = 0; i < 4; i++) {
        const lote = backup.slice(i * elementosPorLote, (i + 1) * elementosPorLote);
        console.log(lote, elementosPorLote);
    }
}
